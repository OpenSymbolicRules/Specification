...
**System:** Any CAS implementing OSR-style symbolic rewriting
**Inspired by:** OSR 4.16+ / OSR 5 (Albert Rich)
...
capable of representing OSR-style symbolic symbolic rules, independently of
...
Today, OSR rules exist only in Mathematica-native formats (.nb, .m). Every port
(SymJa, SymPy) reparses those files with ad-hoc converters. This specification
aims to be the **first language-neutral interchange format** for symbolic rule
knowledge, covering the full set of ~7,800 rules and 72,000+ test problems.

> **Note on rule count.** OSR 4.16 contained approximately 6,700 rules.
> The ongoing OSR 5 redesign has expanded this to approximately 7,800 rules.
...
While OSR-Expr is aligned with MathLive MathJSON on core operator names,
the following architectural differences exist and are intentional:

| Aspect | MathLive MathJSON | OSR-Expr | Rationale |
|--------|-------------------|-----------|-----------|
...
| **Inverse trig** | Compositional: `["Apply", ["InverseFunction", "Sin"], "x"]` | Direct operator: `["Asin", "x"]` | OSR rules reference `ArcSin` as atomic function; compositional form would complicate pattern matching |
...
| Term | Definition |
|------|-----------|
| **OSR** | Portable Integration Rules Format — the format specified herein |
| **OSR-Expr** | The expression sub-language defined by this specification for mathematical expressions |
| **Rule** | Triple (pattern, constraints, result) describing an integral transformation |
| **Pattern** | OSR-Expr with wildcards describing the expression form |
| **Constraint** | OSR-Expr predicate over bound variables (FreeQ, IntegerQ, etc.) |
| **Result** | OSR-Expr of the antiderivative or recursive call to the integrator |
...
| **Taxonomy** | Hierarchy of rule categories matching OSR's 9-section structure |
| **Test suite** | Set of (expression, expected antiderivative) pairs for validation |
| **Content Dictionary (CD)** | OpenMath mechanism assigning semantics to symbols |
| **Phrasebook** | Adapter translating between OSR-Expr and a host CAS internal representation |
...
## 4. Requirements — OSR-Expr expression language

### 4.1 Formal grammar

**OSR-X-001** [Ubiquitous]
...
> The `$` character is permitted in symbol names to accommodate OSR's
> internal fluid variables (e.g. `$base$`, `$expon$`).

**OSR-X-002** [Ubiquitous]
The OSR-Expr language shall be a strict subset of JSON — every valid OSR-Expr shall be a valid JSON value.

**OSR-X-003** [Ubiquitous]
The OSR-Expr language shall be self-contained: its grammar, operator catalogue, and semantics shall be fully defined within this specification, without normative reference to any external format.
...
**OSR-X-010** [Ubiquitous]
OSR-Expr shall define the following **core arithmetic operators**:
...
**OSR-X-011** [Ubiquitous]
OSR-Expr shall define the following **trigonometric function operators** (the full set of 6 circular functions):
...
**OSR-X-012** [Ubiquitous]
OSR-Expr shall define the following **hyperbolic function operators** (the full set of 6 hyperbolic functions):
...
**OSR-X-013** [Ubiquitous]
OSR-Expr shall define the following **inverse trigonometric function operators** (the full set of 6 inverse circular functions):
...
> (a) OSR rules reference `ArcSin[x]` as an atomic function head;
> (b) pattern matching on `["Asin", "x_"]` is simpler than on a nested
> `["Apply", ["InverseFunction", ...], ...]` structure;
> (c) the Mathematica-to-OSR converter maps `ArcSin` → `Asin` directly.
...
**OSR-X-014** [Ubiquitous]
OSR-Expr shall define the following **inverse hyperbolic function operators** (the full set of 6 inverse hyperbolic functions):
...
**OSR-X-015** [Ubiquitous]
OSR-Expr shall define the following **exponential and logarithmic operators**:
...
**OSR-X-016** [Ubiquitous]
OSR-Expr shall define the following **special function operators** required by OSR sections 8.1–8.10:
...
> `BesselK`) are included to cover OSR section 8.10. Although this section
...
**OSR-X-017** [Ubiquitous]
OSR-Expr shall define the following **mathematical constant symbols**:
...
**OSR-X-018** [Ubiquitous]
OSR-Expr shall define the following **integration-specific operators** with no equivalents in existing standards:
...
**OSR-X-019** [Ubiquitous]
OSR-Expr shall define the following **utility function operators** used in
OSR rule results and constraints. These are algorithmic functions that each
...
| `Rt` | 2 | OSR's n-th root choosing the simplest real/complex form |
...
> OSR temporarily deactivates trig functions during pattern matching to
> prevent premature CAS evaluation. Sections 4.7.5 ("Inert trig functions")
> and 4.7.9 ("Active trig functions") in the OSR rule set contain the
...
| `Dif` | 2 | OSR's own symbolic derivative (distinct from D; avoids built-in side effects) |
...
| `Mods` | 2 | Modular arithmetic function used in OSR rules |

### 4.12 Operator catalogue — Structural/utility operators

**OSR-X-020** [Ubiquitous]
OSR-Expr shall define the following **structural/utility operators**:
...
**OSR-X-025** [Optional feature]
Where a host system defines additional operators beyond the OSR-Expr catalogue, the host system shall declare them in the `"extensions"` field of `meta.json` with their name, arity, and informal semantics.

**OSR-X-026** [Unwanted behaviour]
If a OSR-Expr expression contains an operator not present in the core catalogue and not declared in `meta.json` extensions, then the Loader shall emit a warning and treat the operator as an opaque function symbol.

### 4.14 Compatibility and mapping

**OSR-X-030** [Ubiquitous]
The specification shall include a normative **Annex A** providing bidirectional mapping tables between OSR-Expr operators and:
  (a) MathLive MathJSON Standard Library operator names,
  (b) OpenMath Content Dictionary symbols,
  (c) Content MathML elements.

**OSR-X-031** [Ubiquitous]
For core arithmetic and direct trigonometric operators (excluding inverse trigonometric functions), OSR-Expr operator names shall be identical to MathLive MathJSON operator names to maximise de facto compatibility.

**OSR-X-031a** [Ubiquitous]
For inverse trigonometric and inverse hyperbolic functions, OSR-Expr shall use shortened names (`Asin`, `Acos`, `Atan`, `Asinh`, etc.) rather than the MathJSON compositional form. The Annex A mapping table shall document this divergence explicitly.

**OSR-X-032** [Ubiquitous]
This specification shall clearly state that OSR-Expr is a self-contained format: compliance does not require any MathJSON, OpenMath, or MathML implementation.

**OSR-X-033** [Optional feature]
Where a Phrasebook (adapter) to OpenMath is provided, the Phrasebook shall map OSR-Expr operators to the OpenMath CD symbols listed in the Annex A mapping table.

**OSR-X-034** [Optional feature]
Where a Phrasebook to Content MathML is provided, the Phrasebook shall map OSR-Expr operators to the Content MathML elements listed in the Annex A mapping table.

### 4.15 Versioning and stability

**OSR-X-040** [Ubiquitous]
The OSR-Expr operator catalogue shall be versioned independently using SemVer, within the overall schema version.

**OSR-X-041** [Ubiquitous]
Removal of an operator from the core catalogue shall constitute a breaking change and shall increment the major version.

**OSR-X-042** [Ubiquitous]
Addition of new operators to the core catalogue shall constitute a minor change and shall increment the minor version.

**OSR-X-043** [Ubiquitous]
In case of divergence between a future MathLive MathJSON version and OSR-Expr, the OSR-Expr definition shall take precedence for compliant implementations.
...
> **Design note — Load-order priority.** OSR's `Rubi.m` states: *"The order
...
Rule files shall be organised in a directory tree reflecting the OSR taxonomy (algebraic, exponential, logarithmic, trigonometric, etc.).

**OSR-F-011** [Ubiquitous]
...
> OSR's `Rubi.m` defines `$LoadElementaryFunctionRules` and
> `$LoadShowSteps` as boolean flags controlling which rule sections are
> loaded. OSR models this via `"feature_flags"` in `meta.json`:
> ```json
> {
>   "feature_flags": {
...
─────────────────       ──────────────         ────────────────
  *.json (§5.1)  ──▶  osr-compile   ──▶  *.osr.cbor (§5.5)
...
**OSR-B-004** [Ubiquitous]
CBOR-encoded files shall use the file extension `.osr.cbor`.

**OSR-B-005** [Ubiquitous]
...
**OSR-B-006** [Event-driven]
When the `osr-compile` tool receives a directory of JSON rule files,
it shall produce one `.osr.cbor` file per JSON source file, preserving
the directory structure.

**OSR-B-007** [Optional feature]
Where a single-file archive is preferred, the `osr-compile` tool shall
produce a single `.osr.cbor` file containing a CBOR map with section
paths as keys and rule arrays as values.

**OSR-B-008** [Event-driven]
When the Loader detects a `.osr.cbor` file alongside its `.json` source,
the Loader shall compare modification timestamps and load the newer file.

**OSR-B-009** [Unwanted behaviour]
If a `.osr.cbor` file is older than its `.json` source, then the Loader
shall ignore the CBOR file, load the JSON source, and emit a warning
recommending recompilation.

**OSR-B-010** [Unwanted behaviour]
If a `.osr.cbor` file fails integrity validation (truncated, corrupted,
or schema-version mismatch), then the Loader shall fall back to the JSON
source and emit a warning.

**OSR-B-011** [Event-driven]
When the `osr-compile` tool produces a `.osr.cbor` file, it shall embed
a CBOR map header containing: `osr_version` (string), `source_sha256`
(hex string of the JSON source SHA-256), `compiled_at` (ISO 8601 timestamp),
and `rule_count` (integer).

**OSR-B-012** [Ubiquitous]
...
**OSR-B-013** [Optional feature]
Where streaming loading is supported, the `osr-compile` tool shall optionally
...
**OSR-B-014** [Ubiquitous]
OSR-Expr expressions within CBOR files shall use the same array-based
...
**OSR-B-015** [Optional feature]
Where string interning is supported, the `osr-compile` tool shall use
...
> **Design note.** OSR uses Mathematica's `Optional` with arbitrary
...
(`m`, `n`, `p`, `q`) default to 1. This convention matches OSR's most common
...
> **Design note.** Some OSR rules use Mathematica's `__` and `___` patterns
...
**OSR-C-011** [Ubiquitous]
The format shall support the following **integer comparison predicates** (OSR-specific, combining integer test with comparison):
...
**OSR-C-023** [Ubiquitous]
The format shall support the following **function-of predicates** used by OSR
...
> **Design note.** These predicates are central to OSR's substitution
...
**OSR-T-001** [Ubiquitous]
The taxonomy shall follow the **actual OSR hierarchy** with 9 main categories,
matching the directory structure in the OSR IntegrationRules repository:
...
> **Design note — Taxonomy.** This taxonomy matches OSR's actual directory
...
**OSR-T-001a** [Ubiquitous]
Note that OSR uses **even-numbered subsections for the co-function rules**
...
**OSR-E-040** [Event-driven]
When the Engine begins processing a trigonometric or hyperbolic expression,
...
> **Design note.** This mechanism is essential to OSR's correctness.
> Without inert forms, the CAS may simplify `Sin[x]^2 + Cos[x]^2` to `1`
> before pattern matching can identify the appropriate rule. OSR sections
...
**OSR-V-001** [Ubiquitous]
The format shall include a test suite as JSON files separate from rule files.

**OSR-V-002** [Ubiquitous]
Each test problem shall contain: `id`, `expression` (OSR-Expr), `variable` (string), `optimal_antiderivative` (OSR-Expr), and `num_steps` (integer).

**OSR-V-003** [Event-driven]
...
**OSR-V-004** [Event-driven]
When the result differs, the validator shall grade per OSR scale: A (≤ 2× optimal), B (> 2× but correct), C (correct, sub-optimal), F (incorrect/timeout).

**OSR-V-005** [Ubiquitous]
The test suite shall cover a minimum of 72,000 problems.

**OSR-V-006** [Unwanted behaviour]
...
**OSR-P-001** [Ubiquitous]
The format shall be independent of any programming language — no rule file shall contain executable code.

**OSR-P-002** [Ubiquitous]
The format shall use exclusively native JSON data types.

**OSR-P-003** [Ubiquitous]
The format shall be parsable by any standard JSON parser without extensions.

### 13.2 OSR compatibility

**OSR-P-010** [Ubiquitous]
The format shall represent the entirety of OSR's rule set (~7,800 rules) without loss of information.

**OSR-P-011** [Ubiquitous]
The format shall preserve OSR rule numbering (`id` field) for traceability.

**OSR-P-012** [Ubiquitous]
The format shall represent all OSR predicates (FreeQ, IntegerQ, IGtQ, NonzeroQ, PolynomialQ, TrigQ, FunctionOfQ, FunctionOfExponentialQ, etc.).

**OSR-P-013** [Ubiquitous]
The format shall represent all OSR utility functions (functions, ExpandIntegrand, NormalizeIntegrand, SimplifyAntiderivative, Rt, ActivateTrig, DeactivateTrig, etc.).

**OSR-P-014** [Ubiquitous]
The format shall represent all mathematical functions appearing in OSR rule results, including special functions (PolyLog, EllipticF, Erf, Gamma, Hypergeometric2F1, BesselJ, etc.).

**OSR-P-015** [Ubiquitous]
The format shall preserve the exact rule loading order defined in OSR's
`Rubi.m` via the `"load_order"` manifest in `meta.json`.

### 13.3 Phrasebook architecture

**OSR-P-020** [Ubiquitous]
Each host CAS integration shall provide a **Phrasebook** — an adapter module translating between OSR-Expr and the host CAS internal representation.

**OSR-P-021** [Ubiquitous]
...
**OSR-P-023** [Optional feature]
Where an OpenMath Phrasebook is provided, it shall map OSR-Expr operators to OpenMath CD symbols per Annex A.

**OSR-P-024** [Optional feature]
Where a Content MathML Phrasebook is provided, it shall map OSR-Expr operators to Content MathML elements per Annex A.

**OSR-P-025** [Optional feature]
...
**OSR-D-001** [Ubiquitous]
The format shall be documented by a validatable JSON Schema (draft-07 or later).

**OSR-D-002** [Ubiquitous]
Rule files converted from OSR shall cite the OSR Software License (MIT) in `meta.json` `"license"`.
...
|----------|------------|-------|
| OSR-Expr language (X) | OSR-X-001 to X-043, X-031a | 27 |
| File format (F) | OSR-F-001 to F-014, F-009a/b/c, F-012a | 18 |
| Binary serialisation (B) | OSR-B-001 to B-031 | 20 |
| Wildcards (W) | OSR-W-001 to W-006, W-002a | 7 |
| Constraints (C) | OSR-C-001 to C-041, C-023 | 23 |
| Taxonomy (T) | OSR-T-001 to T-004, T-001a | 5 |
| Loader (L) | OSR-L-001 to L-022 | 11 |
| Engine (E) | OSR-E-001 to E-041 | 14 |
| Step-by-step (S) | OSR-S-001 to S-003 | 3 |
| Validation (V) | OSR-V-001 to V-006 | 6 |
| Portability (P) | OSR-P-001 to P-032, P-015, P-022a | 18 |
| Documentation (D) | OSR-D-001 to D-003 | 3 |
| Non-functional (N) | OSR-N-001 to N-004 | 4 |
| **Total** | | **159** |
...
| OSR-Expr | MathLive MathJSON | OpenMath CD#symbol | Content MathML |
|-----------|-------------------|-------------------|----------------|
...
| OSR-Expr | MathLive MathJSON | OpenMath CD#symbol | Content MathML | Notes |
|-----------|-------------------|-------------------|----------------|-------|
...
> OSR uses atomic operators for the reasons stated in §4.5.
...
### A.7 Integration-specific operators (OSR-only)

| OSR-Expr | Semantics | OpenMath equivalent | MathML equivalent |
|-----------|-----------|--------------------|--------------------|
